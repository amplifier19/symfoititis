...
const course = computed(() => {
  return (
    courses.value.find((c: Course) => c.c_id === props.booking.c_id) || {
      c_id: -1,
      dep_id: -1,
      c_display_name: "",
      semester: -1,
...
<template>
  ...
  >
    ...
    <template
      v-else-if="
        chatStats.some(
          (s) => s.room == props.booking.room && getUnreadMessages() > 0,
        )
      "
      v-slot:card-header
    >
      <div class="unread-message-badge">
        <span class="sm-font unread-message-text card-top-text">{{
          getUnreadMessages() > 9 ? "9+" : getUnreadMessages()
        }}</span>
      </div>
    </template>
  ...
  <section v-else class="booking-row">
    <div id="course-details" class="details">
      <span class="course-title booking-field lg-font md-fw">{{
        course.c_display_name
      }}</span>
      <span class="semester booking-field lg-font lt-fw"
        >{{ alphabet[course.semester - 1] }}' Εξάμηνο</span
      >
    </div>
    <div id="booking-details" class="details">
      <span class="time booking-field md-font sb-fw"
        >{{ booking.start_time }}:00 - {{ booking.start_time + 1 }}:00</span
      >
      <span v-if="isStudent" class="name booking-field md-font lt-fw">
        {{ booking.teacher_firstname }} {{ booking.teacher_lastname }}
      </span>
      <span v-else class="name booking-field md-font lt-fw">{{
