...
export const useFetch = () => {
  ...
  const universitiesDataService =
    UniversitiesDataService.getUniversitiesDataFactory();
  const departmentsDataService =
    DepartmentsDataService.getDepartmentsDataFactory();
  ...
  const createUniversity = async (event: Event) => {
    ...
    const uni_display_name =
      formData.get("uni_display_name")?.toString().trim() || "";
  ...
  const deleteUniversity = async (event: Event) => {
    ...
    await universitiesDataService.deleteUniversity(
      parseInt(uni_id),
      uni_alt_name,
    );
  };
  ...
  const createDepartment = async (event: Event) => {
    ...
    const uni_id = formData.get("uni_id")?.toString() || "";
    const dep_display_name =
      formData.get("dep_display_name")?.toString().trim() || "";
  ...
  const deleteDepartment = async (event: Event) => {
    ...
    await departmentsDataService.deleteDepartment(
      parseInt(dep_id),
      dep_alt_name,
    );
  };
  ...
  const createCourse = async (event: Event) => {
    ...
    const c_display_name =
      formData.get("c_display_name")?.toString().trim() || "";
  ...
  const createNote = async (event: Event) => {
    ...
    const note_display_name =
      formData.get("note_display_name")?.toString().trim() || "";
    const note_filename =
      formData.get("note_filename")?.toString().trim() || "";
  ...
  const generatePresignedUrl = async (c_id: number, filename: string) => {
    const attachmentUrl = await fileDataService.generatePresignedUrl(
      c_id,
      filename,
    );
