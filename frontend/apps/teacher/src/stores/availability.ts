...
export const useAvailabilityStore = defineStore("availabilityStore", () => {
  ...
  const filteredByDateAvailabilitySlots = computed(() => {
    return availabilitySlots.value.filter((slot) => {
      return (
        ...
        !cancelableAvailabilitySlotIds.value.includes(slot.av_id!)
      );
    });
  });
  const filteredInsertableAvailabilitySlots = computed(() => {
    return insertableAvailabilitySlots.value.filter(
      (slot) => slot.start_time > 0,
    );
  });

  const updateUpdatableSlot = (slotKey: number, startTime: number) => {
    const i = availabilitySlots.value.findIndex(
      (slot) => slot.av_id === slotKey,
    );
    if (i < 0) return;
    const j = updatableAvailabilitySlots.value.findIndex(
      (slot) => slot.av_id === slotKey,
    );
    const modifiedAndNotInUpdatableList =
      availabilitySlots.value[i].start_time != startTime && j < 0;
    const modifiedAndExistsInUpdatableList =
      availabilitySlots.value[i].start_time != startTime && j >= 0;
    const notModifiedAndExistsInUpdatableList =
      availabilitySlots.value[i].start_time === startTime && j >= 0;
  ...
  const cancelUpdatableSlot = (av_id: number) => {
    const idx = availabilitySlots.value.findIndex(
      (slot) => slot.av_id === av_id,
    );
    if (idx < 0) return;
    cancelableAvailabilitySlotIds.value = [
      ...cancelableAvailabilitySlotIds.value,
      availabilitySlots.value[idx].av_id!,
    ];
  };
  ...
  const getAvailabilityPreferences = (day: number) => {
    if (filteredByDateAvailabilitySlots.value.length == 0) {
      const slot = availabilitySlots.value.findLast(
        (el: AvailabilitySlot) =>
          day === el.week_day && selectedDate.value !== el.date,
      );
      if (!!slot) {
        availabilityPreferences.value = JSON.parse(
          JSON.stringify(
            availabilitySlots.value.filter(
              (el) => day === el.week_day && slot.date === el.date,
            ),
          ),
        );
        availabilityPreferences.value.forEach(
