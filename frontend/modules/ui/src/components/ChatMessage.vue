...
<template>
  <section
    v-if="XOR(isTeacher, props.chatMessage.is_teacher)"
  ...
  <section v-else class="message-container" id="sender-message-container">
    ...
    <div
      v-else-if="
        chatStats.some(
          (s) => s.otherLastReadMessageId == props.chatMessage.message_id,
        )
      "
      class="arrow-cnt"
    >
    ...
    <div
      v-else-if="
        props.chatMessage.state == 'sent' &&
        (props.chatMessage.message_id || 0) >
          (chatStats.find((s) => s.room == props.chatMessage.room)
