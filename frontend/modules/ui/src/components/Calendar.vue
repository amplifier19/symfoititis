...
const initWeeks = () => {
  ...
  for (let i = 1, j = first.getDay(); i <= monthsDays[month.value]; i++, j++) {
    ...
    if (isToday) {
      weeks.value[week][weeks.value[week].length - 1].cell_classes.push(
        "pf-m-current",
      );
    }
    ...
    if (isPast) {
      ...
      weeks.value[week][weeks.value[week].length - 1].btn_classes.push(
        "notallowed-btn",
      );
    }
...
.pf-v5-c-calendar-month__date:focus {
  --pf-v5-c-calendar-month__date--focus--after--BorderColor: var(
    --orange-shadow
  );
