...
export class AvailabilityApiService {
  ...
  public getAvailabilitySlots = async (cid: number) => {
    return await fetch(
      `${this.API_BASE_URL}/tutoring/availability/${cid}/teacher/${this.userStore.profile.id}`,
      {
        method: "GET",
      },
    );
  ...
  public addAvailabilitySlots = async () => {
    return await fetch(
      `${import.meta.env.VITE_API_BASE}/tutoring/availability`,
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify(
          this.availabilityStore.filteredInsertableAvailabilitySlots,
        ),
      },
    );
  };
