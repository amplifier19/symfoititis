...
import { useCourseStore, useErrorStore, useUserStore } from "@symfoititis-frontend-monorepo/stores"
...
export const useCoursesDataService = () => {
    ...
    const getAvailableTutoringCourseIds = async () => {
        try {
            const response = await coursesApiService.getAvailableTutoringCourseIds(department.value.dep_id)
            const data = await response.json()
            if (!!data.error) {
                errorStore.addError(data)
                return
