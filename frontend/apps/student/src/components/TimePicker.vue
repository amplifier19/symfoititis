...
const filteredAvailabilitySlots = computed(() => {
  return availabilityStore.availabilitySlots.filter(
...
const handleRefresh = async () => {
  loading.value = true;
  await availabilityDataService.getAvailabilitySlots(
    c_id.value,
    props.teacherId,
    true,
  );
...
<template>
  ...
  <div v-else class="time-picker-container">
    <ul
      ref="timeSlotList"
      v-if="filteredAvailabilitySlots.length > 0"
      class="time-picker"
    >
      <li class="date-prompt">
        <span class="date-prompt-text"
          >{{ weekDays[props.selectedDay.weekDay] }},
          {{ props.selectedDay.date }}</span
        >
        <RefreshButton @refresh="handleRefresh" />
      </li>
      <li
        v-for="slot in filteredAvailabilitySlots"
        @click="addTimeSlot($event, slot.av_id!)"
      >
      ...
      <div
        v-if="selectedAvailabilitySlotIds.length > 0"
        @click="bookSlots"
        class="booking-btn"
        id="book-btn"
      >
    ...
    <div v-else-if="!selectedDay.date" class="time-picker time-picker-prompt">
      <span>Επελεξε μια διαθεσιμη ημερομηνια </span>
      <i class="fa fa-circle"></i>
    </div>
    <div v-else class="time-picker time-picker-prompt">
      <span
        >Απ' οτι φαινεται ο συμφοιτητης ειναι αρκετα busy. Επελεξε αλλη
        ημερομηνια</span
      >
    </div>
  </div>
