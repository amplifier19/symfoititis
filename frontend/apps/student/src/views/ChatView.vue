...
const { connectToStompServer, getMessages, getChatStats } =
  useChatDataService();
...
const course = computed(() => {
  return (
    courses.value.find((c: Course) => c.c_id === c_id.value) || {
      dep_id: -1,
      semester: -1,
      c_display_name: "",
    }
...
const booking = computed(() => {
  return (
    bookings.value.find((b: Booking) => b.b_id === b_id.value) || {
      av_id: -1,
      c_id: -1,
      s_id: "",
      t_id: "",
...
onMounted(async () => {
  ...
  await getCourses();
  await getBookings();
  await getMessages(c_id.value, booking.value.t_id);
