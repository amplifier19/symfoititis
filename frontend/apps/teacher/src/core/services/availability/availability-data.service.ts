...
export class AvailabilityDataService {
    ...
    public getAvailabilitySlots = async (cid: number) => {
        try {
            const response = await this.availabilityApiService.getAvailabilitySlots(cid)
    ...
    public addAvailabilitySlots = async () => {
        if (this.availabilityStore.filteredInsertableAvailabilitySlots.length === 0) return
        try {
    ...
    public updateAvailabilitySlots = async () => {
        ...
        try {
            const response = await this.availabilityApiService.updateAvailabilitySlots()
            const data = await response.json()
    ...
    public cancelAvailabilitySlots = async () => {
        if (this.availabilityStore.cancelableAvailabilitySlotIds.length === 0) return
        try {
            const response = await this.availabilityApiService.cancelAvailabilitySlots()
            const data = await response.json()
            if (!!data.error) {
